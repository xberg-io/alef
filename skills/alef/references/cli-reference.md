...
`alef generate` finalises the embedded hash _after_ every formatter has run (rustfmt, rubocop, dotnet format, spotless, oxfmt, oxlint, mix format, php-cs-fixer, ruff, cargo-sort, ...), so the on-disk hash always describes the actual on-disk byte content.
