...
//!
//! These tests exercise the real `KotlinBackend::generate_bindings` path end to end -- not just
...
fn generate_bindings_writes_back_the_mutated_record_end_to_end() {
    ...
    let facade = &files
        .iter()
        .find(|f| f.content.contains("tagRecord"))
        .expect("the Kotlin JVM facade file declaring tagRecord must be generated")
        .content;

    assert!(
        facade.contains("fun tagRecord(record: Record): Record {"),
        "must return the updated value instead of Unit, got:\n{facade}"
    );
    assert!(
        facade.contains("return Bridge.tagRecord(record)"),
        "must delegate to and return the Java Bridge method's (now non-void) result, got:\n{facade}"
    );
    assert!(
        !facade.contains("): Unit {") || !facade.contains("tagRecord"),
        "must not regress to the lossy Unit-returning shape for tagRecord, got:\n{facade}"
    );
}
