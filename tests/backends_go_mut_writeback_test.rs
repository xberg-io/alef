...
//!
//! These tests exercise the real `GoBackend::generate_bindings` path end to end -- not just the
//! shared `codegen::mut_writeback` policy module in isolation -- so they prove the backend is
//! actually wired to the policy, not merely that the policy itself is correct.
...
[crates.go]
module = "github.com/test/krz-lib"
"#,
    )
}
...
fn generate_bindings_writes_back_the_mutated_record_end_to_end() {
    ...
    assert!(
        content.contains("func TagRecord(record Record) (*Record, error) {"),
        "must return the updated value instead of a bare error, got:\n{content}"
    );
    assert!(
        content.contains("C.krz_tag_record(cRecord)"),
        "must still call the FFI mutator, got:\n{content}"
    );
    assert!(
        content.contains("C.krz_record_to_json(cRecord)"),
