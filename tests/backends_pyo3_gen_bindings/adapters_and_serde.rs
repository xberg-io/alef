...
/// End-to-end proof that `Pyo3Backend::generate_bindings` wires a delegating `Deserialize` for
/// a struct carrying a container-level `#[serde(from/into)]` conversion. pyo3's decision itself
/// lives in `codegen::generators::structs` (already covered at the unit level in
/// `structs/tests.rs`), but the *wiring* -- `cfg.delegate_deserialize_to_core_for_types =
/// Some(&core_to_binding_for_default)` in `pyo3/gen_bindings/mod.rs` -- is only exercised here,
/// through the real backend entry point. ~keep
#[test]
fn generate_bindings_delegates_deserialize_for_a_sound_container_conversion_struct() {
    ...
    let api = ApiSurface {
        ...
        types: vec![TypeDef {
            ...
            fields: vec![
                make_field("x", TypeRef::Primitive(PrimitiveType::F64), false),
                make_field("y", TypeRef::Primitive(PrimitiveType::F64), false),
            ],
            has_serde: true,
