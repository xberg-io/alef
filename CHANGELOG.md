...
- fix(e2e-wasm): inject async init() call in setup.ts for Node.js test environments. vitest running wasm-pack output requires explicit import and await of the init() default export before any WASM functions are called, preventing `TypeError: Cannot read properties of undefined (reading '__wbindgen_add_to_stack_pointer')` at test startup.
