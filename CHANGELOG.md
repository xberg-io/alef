...
- **alef-backend-jni: migrate generated shims to jni 0.22 `EnvUnowned`/`Env` split**: jni 0.22 split `JNIEnv` into FFI-safe `EnvUnowned<'frame>` (kept under the `JNIEnv` alias) and `Env<'_>`, which carries the real API surface (`get_string`, `new_string`, `throw_new`, `convert_byte_array`, `byte_array_from_slice`). Generated shims used the deprecated alias for both signature and body, tripping `E0599: no method named 'get_string' / 'new_string' / 'throw_new' found for &mut EnvUnowned`. The codegen now emits `use jni::{Env, JNIEnv};`, switches helper signatures (`jstring_to_string`, `string_to_jstring`, `throw_jni_error`, `run_or_throw`) from `&mut JNIEnv` to `&mut Env<'_>`, and wraps every extern shim body (function shim, method shim, streaming start/next shims) in `env.with_env(|env| -> RetTy { ... })` so the inner `env` is the upgraded `&mut Env<'_>`. `_env`-only shims (destructors, streaming free) are unchanged. Surfaced on html-to-markdown's prek run (`crates/html-to-markdown-rs-jni`). Snapshots updated; existing 25-test suite passes. (`crates/alef-backend-jni/src/gen_shims.rs`, `crates/alef-backend-jni/tests/{gen_shims_test.rs,snapshots/*.snap}`)
