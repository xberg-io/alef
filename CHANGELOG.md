...
- fix(alef-backend-magnus): delegate `&self` methods through `Mutex` on opaque types that also have `&mut self` methods. Opaque types with both `&self` and `&mut self` methods are Mutex-wrapped; the codegen now correctly emits `.lock().unwrap()` for `&self` method delegation when `has_mut_methods` is true, matching the NAPI pattern. Previously, only `&mut self` methods got the lock, leaving `&self` methods to incorrectly call `.method()` directly on `Arc<Mutex<T>>` instead of `.lock().unwrap().method()`.
