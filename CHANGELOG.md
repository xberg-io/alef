...
- **alef-scaffold: suppress php-cs-fixer runtime-version advisories in generated config**: PHP package scaffolds now set `setUnsupportedPhpVersionAllowed(true)` in `.php-cs-fixer.dist.php` and stop using the deprecated `PHP_CS_FIXER_IGNORE_ENV` composer-script prefix. (`crates/alef-scaffold/src/languages/php.rs`, `crates/alef-scaffold/src/tests.rs`)
