...
- fix(go-e2e-codegen): `file_path` args in test fixtures are now resolved to absolute paths using `filepath.Join` and `filepath.Abs`, similar to the existing Rust e2e codegen fix. Previously, relative fixture paths like `"pdf/fake_memo.pdf"` were passed verbatim to `extract_file*` functions, causing failures when the test ran from a different working directory. Now the paths are resolved relative to the repo-root `test_documents/` directory at test runtime.
