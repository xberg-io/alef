...
- **alef-cli: make post-generation formatter warnings actionable**: formatter failures now include both stdout and stderr instead of dropping stdout-only diagnostics. Python package formatting is scoped to the generated stub package when configured, avoiding unrelated hand-written package tests, and PHP formatting sets `PHP_CS_FIXER_IGNORE_ENV=1` to suppress php-cs-fixer's runtime-version advisory for generated code. (`crates/alef-cli/src/pipeline/format.rs`)
