...
- **alef-backend-csharp: emit DTOs as C# 9+ `sealed record` with property bodies**: Previously, DTOs were emitted as `public sealed class Foo { public T Field { get; set; } = default; }` (C# 6 style). Now the backend emits `public sealed record Foo { [property: JsonPropertyName("...")] T Field { get; set; } = default; }` — idiomatic C# 9+ record syntax with `[property:]` attribute targets for JSON metadata. This improves consistency with modern C# conventions and enables compiler-driven immutability checking. Internally-tagged enums (with data variants) now use abstract `record` bases with `[JsonDerivedType]` discriminator annotations instead of flat class hierarchies, enabling type-safe sealed-union semantics via inheritance. Enum XML doc comments are now properly formatted with explicit `</summary>` closing tags. (`crates/alef-backend-csharp/templates/record_class_header.jinja`, `crates/alef-backend-csharp/templates/enum_header.jinja`)
