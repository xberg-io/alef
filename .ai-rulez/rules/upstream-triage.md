...
- **Alef codegen bug** (issue reproduces in alef's own e2e tests or `alef-extract` misses an item): fix in `../alef`, update `CHANGELOG.md` `[Unreleased]`, commit normally, prepare a release per `release-procedure` skill.
