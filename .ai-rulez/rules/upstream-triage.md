...
- **Alef-owned workflow/action bug** (an Alef-maintained publish workflow, pre-commit hook, or scaffold action has an error): fix the owning Alef workflow/action repository, commit, then retag only the documented action tags for that repository. Do not send unrelated downstream workflow issues to a hard-coded actions repository.
