...
- **Never force-push to shared branches** (`main`, `master`, release branches). The only exception: retagging the two version tags in `../actions` (`v1.0.0` and `v1`) when a critical action fix ships.
