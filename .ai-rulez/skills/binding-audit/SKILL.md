...
For each language under `packages/<lang>/`:
...
grep -E "export (function|class|type|const) " packages/typescript/src/index.ts

# Ruby:
grep -E "^  def " packages/ruby/lib/*.rb

# Go (FFI):
grep -E "^func " packages/go/*.go

# Java (JNI):
grep -E "^\s+(public static|public) (native )?" packages/java/*/src/main/java/*/*.java

# C#:
grep -E "^\s+public (static|extern)" packages/csharp/src/*.cs
```
...
| Bindings | `grep -E "export\|def\|func " packages/<lang>/*` | Per-language sets |
