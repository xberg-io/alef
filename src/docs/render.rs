    ...
    for (group, template, relative_output) in groups {
        ...
        for root in &cfg.outputs {
            ...
            rendered.push(GeneratedFile {
                path: output,
                content: with_html_header(content.clone(), "alef docs"),
                generated_header: true,
            });
        }
    }
...
fn markdown_header(title: &str) -> String {
    ...
    with_html_header(out, "alef docs")
}

/// Prepend an HTML-comment `alef:hash:`-style marker to Markdown content that
/// cannot carry the usual `//`/`#`/`/* */` header (no `CommentStyle` renders
/// HTML comments; see `write::marker_comment_style`'s doc for why `.md` is
/// treated as unstampable there). Used for both docs pages and README output
/// so both prove ownership to the write-time guard purely from content,
/// identically on a fresh clone and a warm dev machine -- see
/// `write_scaffold_files_report`'s guard doc for why that distinction
/// matters. `regenerate_command` names the actual CLI invocation that
/// produces this specific file (`"alef docs"`, `"alef readme"`, ...) so the
/// embedded instructions stay accurate per caller. ~keep
pub(crate) fn with_html_header(content: String, regenerate_command: &str) -> String {
