...
/// Additional marker spellings that do not contain [`MARKER_SUBSTRING`]. The
/// regeneration spelling also requires a comment header and "do not edit":
/// handwritten instructions about regenerating other files do not grant ownership.
///
...
pub(super) fn line_has_marker(line: &str) -> bool {
    contains_unnegated_ignore_ascii_case(line, MARKER_SUBSTRING)
        || ALTERNATE_MARKER_SUBSTRINGS.iter().any(|alternate| {
            (*alternate != "regenerate with `alef" || is_regeneration_header(line))
                && contains_unnegated_ignore_ascii_case(line, alternate)
        })
}

// Regeneration advice occurs in handwritten prose too. Match the shipped R
// entrypoint's explicit ownership instruction, not the command alone. ~keep
