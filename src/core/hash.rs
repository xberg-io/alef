...
/// output" -- so a file [`e2e_header`] marks must never point the reader at that command.
/// This is the one regenerate command every e2e-generated file (bindings, mock server,
/// project scaffolding, and test sources alike, across every language backend under
/// `src/e2e/codegen`) shares, regardless of `[workspace.generated_header]`: e2e output is
/// not scoped per-language the way `header_for_config`'s override is. ~keep
const E2E_REGENERATE_COMMAND: &str = "alef e2e generate";
...
/// Return the standard alef header for e2e-generated output, naming `alef e2e generate` as
/// the regenerate command instead of [`header`]'s plain `alef generate` -- the command that
/// command's own summary line says structurally cannot produce e2e/test-app output. Every
/// backend under `src/e2e/codegen` should call this instead of [`header`].
pub fn e2e_header(style: CommentStyle) -> String {
    render_header(
        style,
        &header_body(E2E_REGENERATE_COMMAND, DEFAULT_VERIFY_COMMAND, None),
    )
}
