...
///
/// The file uses `generated_header: false` (create-once seed) so `write_scaffold_files`
/// skips it when `.gitattributes` already exists, preserving any hand-added entries.
fn scaffold_gitattributes(config: &ResolvedCrateConfig, languages: &[Language]) -> Vec<GeneratedFile> {
    ...
    for &lang in languages {
        match lang {
            ...
            // Node and Wasm: package_dir() already returns the crate dir
            // (e.g. `crates/{name}-node`), so one entry suffices.
            _ => {
                dirs.insert(config.package_dir(lang));
            }
        }
    ...
    // e2e test suites are always generated by `alef e2e generate` regardless of language.
    dirs.insert("e2e".to_string());

    let mut content = String::from("# Generated by alef scaffold. Re-run `alef scaffold` to update.\n");
    for dir in dirs {
