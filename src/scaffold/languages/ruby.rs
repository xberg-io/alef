...
# locate the crate. Chdir into the crate's manifest directory (where Cargo.toml
# actually lives — `ext/<ext_name>/native`) so the metadata lookup finds it.
MANIFEST_DIR = File.expand_path("ext/{ext_name}/native", GEM_ROOT)
