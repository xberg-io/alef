...
use crate::core::config::{Language, PrecommitConfig, ResolvedCrateConfig};
...
pub(crate) fn generate_pre_commit_config(config: &ResolvedCrateConfig, languages: &[Language]) -> Vec<GeneratedFile> {
    ...
    let clippy_excludes = {
        ...
        for (suffix, lang) in suffixes {
            if has(*lang) {
                excludes.push_str(&crate::scaffold::template_env::render(
    ...
    let yaml = crate::scaffold::template_env::render(
