...
pub(super) fn render_e2e_helpers(alt_host: &str) -> String {
    let before_spawn = r#"// Generated by alef — DO NOT EDIT.
...
"#;
    // Baked from `[crates.e2e] alt_host` at generation time. `Process.start`'s `environment`
    // map is merged on top of the inherited parent environment, so an already-exported
    // ALEF_MOCK_ALT_HOST (checked first, via `??`) still wins over the configured default. ~keep
    let spawn_call = format!(
        r#"  final process = await Process.start(mockServer.path, [
    '${{repoRoot.path}}/fixtures',
  ], environment: {{
    'ALEF_MOCK_ALT_HOST':
        Platform.environment['ALEF_MOCK_ALT_HOST'] ?? '{alt_host}',
  }});
...
"#;
    format!("{before_spawn}{spawn_call}{after_spawn}")
}
...
mod tests {
    ...
    fn e2e_helpers_prefers_alef_e2e_mock_server_env_var_over_the_repo_root_join() {
        let out = render_e2e_helpers("localhost");
        assert!(
    ...
    /// hard-coding it.
    #[test]
    fn e2e_helpers_bakes_configured_alt_host_into_the_mock_server_spawn() {
        let out = render_e2e_helpers("alt-host-from-config.test");
        assert!(
            out.contains("Platform.environment['ALEF_MOCK_ALT_HOST'] ?? 'alt-host-from-config.test'"),
            "startMockServer must set ALEF_MOCK_ALT_HOST to the configured alt_host, got: {out}"
        );
    }
}
