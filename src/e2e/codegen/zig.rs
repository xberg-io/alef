...
mod tests_trait_bridge {
    ...
    /// Verify that binding-excluded Named types map to `[*c]const u8` in Zig stubs,
    /// not to `lib.TypeName` (which would be an undefined reference at compile time).
    #[test]
    fn test_excluded_type_maps_to_c_pointer_in_stub() {
        use crate::core::config::TraitBridgeConfig;
        use crate::core::ir::{MethodDef, ParamDef, ReceiverKind, TypeRef};
        use crate::e2e::fixture::Fixture;
        ...
        let methods = vec![&method];
        let emission = super::emit_test_backend_with_excluded(&bridge, &methods, &fixture, &excluded);

        // Excluded return type must become [*c]const u8, not lib.InternalDocument.
        assert!(
            emission.setup_block.contains("[*c]const u8"),
            "excluded type InternalDocument must map to [*c]const u8, got:\n{}",
            emission.setup_block
        );

        // InternalDocument must not appear as a bare named reference.
        assert!(
        ...
        // Non-excluded Named type params also map to [*c]const u8 (Zig vtable thunk ABI).
        assert!(
            !emission.setup_block.contains("lib.ExtractionConfig"),
            "Named param type must not appear as lib.ExtractionConfig in Zig stubs, got:\n{}",
            emission.setup_block
        );
    }
}
