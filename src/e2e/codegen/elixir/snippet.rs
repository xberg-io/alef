...
pub(super) fn render_snippet_body_with_ir(
    ...
    let (mut setup_lines, mut args, _) = super::args::build_args_and_setup(
        &fixture.input,
        recipe.args,
        super::args::ElixirArgsContext {
            module_path: &module,
            options_type: recipe.options_type,
            options_default_fn: override_config.and_then(|value| value.options_via.as_deref()),
            enum_fields: &HashMap::new(),
            fixture,
            _handle_struct_type: override_config.and_then(|value| value.handle_struct_type.as_deref()),
            _handle_atom_list_fields: &HashSet::new(),
            test_documents_path: &e2e_config.test_documents_relative_from(0),
            adapter_request_type: request_type,
            enums,
            config,
            type_defs,
            force_keyword_args: false,
        },
    );
