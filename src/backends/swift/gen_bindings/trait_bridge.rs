...
fn gen_single_trait_bridge_file(
    ...
    // NOTE: Registration functions are NOT emitted here. They live in the main binding .swift file
...
fn build_adapter_call_expr(
    ...
    // For excluded types, the protocol already marshals them as JSON strings (String return type),
    // so result is already a String and doesn't need further encoding.
    // For other types, pass the result through directly (marshal_ok_result handles encoding).
    let return_expr = "result".to_string();
...
mod tests {
    ...
    /// function. Registration is handled by `emit_trait_bridge_forwarders` in the main binding .swift file
    ...
    fn test_no_register_fn_in_trait_bridge_file() {
        ...
        // in the main binding .swift file and uses SwiftDocumentExtractorBox (not Adapter).
        assert!(
