...
impl Backend for SwiftBackend {
    ...
    fn generate_bindings(&self, api: &ApiSurface, config: &ResolvedCrateConfig) -> anyhow::Result<Vec<GeneratedFile>> {
        ...
        // Emit Plugins.swift containing all 6 Swift{Trait}Box classes for FunctionParam bridges.
        if let Some(plugins_file) = emit_plugins_swift_file(api, config, &rust_bridge_sources) {
            files.push(plugins_file);
        }
...
fn emit_plugins_swift_file(
    api: &ApiSurface,
    config: &ResolvedCrateConfig,
    rust_bridge_dir: &std::path::Path,
) -> Option<GeneratedFile> {
    let trait_bridge_configs: Vec<_> = config
        .trait_bridges
        .iter()
        .filter(|b| {
            b.bind_via == BridgeBinding::FunctionParam
                && !b.exclude_languages.iter().any(|l| l == "swift")
        })
        .collect();

    if trait_bridge_configs.is_empty() {
        return None;
    }
    ...
    content.push_str("// Generated by alef. Do not edit by hand.\n");
    content.push_str("// swift-format-ignore-file\n");
    content.push_str("// This file contains generated FFI glue for inbound plugin trait bridges.\n\n");
    content.push_str("import Foundation\n");
    content.push_str("import RustBridge\n\n");

    // Helper envelopes
    content.push_str("// MARK: - JSON envelope helpers\n\n");
    content.push_str("private enum InboundEnvelope<T: Encodable>: Encodable {\n");
    content.push_str("    case ok(T)\n");
    content.push_str("    case err(String)\n\n");
    content.push_str("    enum CodingKeys: String, CodingKey { case ok, err }\n\n");
    content.push_str("    func encode(to encoder: Encoder) throws {\n");
    content.push_str("        var container = encoder.container(keyedBy: CodingKeys.self)\n");
    content.push_str("        switch self {\n");
    content.push_str("        case .ok(let value): try container.encode(value, forKey: .ok)\n");
    content.push_str("        case .err(let message): try container.encode(message, forKey: .err)\n");
    content.push_str("        }\n");
    content.push_str("    }\n");
    content.push_str("}\n\n");

    content.push_str("private func encodeOkVoidEnvelope() -> RustString {\n");
    content.push_str("    return RustString(\"{\\\"ok\\\":null}\")\n");
    content.push_str("}\n\n");

    content.push_str("private func encodeOkEnvelope<T: Encodable>(_ value: T) -> RustString {\n");
    content.push_str("    do {\n");
    content.push_str("        let payload = InboundEnvelope.ok(value)\n");
    content.push_str("        let data = try JSONEncoder().encode(payload)\n");
    content.push_str("        return RustString(\n");
    content.push_str("            String(data: data, encoding: .utf8) ?? \"{\\\"err\\\":\\\"swift: invalid utf8 in envelope\\\"}\")\n");
    content.push_str("    } catch {\n");
    content.push_str("        return encodeErrEnvelope(\"swift: failed to encode ok envelope: \\(error)\")\n");
    content.push_str("    }\n");
    content.push_str("}\n\n");

    content.push_str("private func encodeErrEnvelope(_ message: String) -> RustString {\n");
    content.push_str("    let escaped = message.replacingOccurrences(of: \"\\\\\", with: \"\\\\\\\\\").replacingOccurrences(\n");
    content.push_str("        of: \"\\\"\", with: \"\\\"\\\")\n");
    content.push_str("    return RustString(\"{\\\"err\\\":\\\"\\(escaped)\\\"}\")\n");
    content.push_str("}\n\n");

    // Emit Swift{Trait}Box classes
    for bridge_cfg in &trait_bridge_configs {
        let trait_name = &bridge_cfg.trait_name;
        if let Some(trait_def) = api.types.iter().find(|t| t.is_trait && t.name == *trait_name) {
            emit_plugin_box_class(&mut content, trait_name, trait_def);
        }
    }

    Some(GeneratedFile {
        path: rust_bridge_dir.join("Plugins.swift"),
        content,
        generated_header: false,
    })
}
...
fn emit_plugin_box_class(content: &mut String, trait_name: &str, trait_def: &TypeDef) {
    let box_name = format!("Swift{trait_name}Box");

    content.push_str(&format!(
        "// MARK: - {trait_name}\n\n\
         /// FFI adapter class for `{trait_name}`. Rust looks up `{box_name}` by name from\n\
         /// the `extern \"Swift\"` block and dispatches calls through `Unmanaged<T>.fromOpaque(...)`.\n\
         public final class {box_name} {{\n\
         \x20   private let inner: {trait_name}\n\n\
         \x20   public init(_ inner: {trait_name}) {{\n\
         \x20       self.inner = inner\n\
         \x20   }}\n\n"
    ));
    ...
    for method in &trait_def.methods {
        ...
        // Build the FFI shim method signature and body
        let mut params = vec!["&self".to_string()];
        for p in &method.params {
            let name = p.name.to_snake_case();
            let ty = if p.optional {
                "Option<RustVec<UInt8>>".to_string() // Simplified for now — most plugin params are simple
            } else {
                match &p.ty {
                    TypeRef::Bytes => "RustVec<UInt8>".to_string(),
                    TypeRef::String => "RustString".to_string(),
                    TypeRef::Primitive(_) => "UInt".to_string(),
                    _ => "RustString".to_string(), // JSON-encoded types default to String
                }
            };
        ...
        let params_str = params.join(", ");
        let return_type = "RustString";

        content.push_str(&format!(
            "    public func {shim_name}({params_str}) -> {return_type} {{\n"
        ));

        // Method body: extract parameters, call inner method, wrap result
        // For simplicity, map RustString to String, RustVec<UInt8> to [UInt8], etc.
        let call_args: Vec<String> = method
            .params
            .iter()
            .map(|p| {
                let name = p.name.to_snake_case();
                match &p.ty {
                    TypeRef::String => format!("{name}.toString()"),
                    TypeRef::Bytes => format!("Array({name})"),
                    TypeRef::Primitive(crate::core::ir::PrimitiveType::Bool) => name,
                    _ => format!("{name}.toString()"), // Default: JSON decode
                }
            })
            .collect();
        let call_args_str = call_args.join(", ");

        // Determine if method is throwing
        let is_throwing = method.error_type.is_some();

        if is_throwing && matches!(method.return_type, TypeRef::Unit) {
            content.push_str(&format!(
                "        do {{\n\
                 \x20\x20\x20\x20try inner.{method_camel}({call_args_str})\n\
                 \x20\x20\x20\x20return encodeOkVoidEnvelope()\n\
                 \x20   }} catch {{\n\
                 \x20\x20\x20\x20return encodeErrEnvelope(\"\\\\(error)\")\n\
                 \x20   }}\n"
            ));
        } else if is_throwing {
            content.push_str(&format!(
                "        do {{\n\
                 \x20\x20\x20\x20let result = try inner.{method_camel}({call_args_str})\n\
                 \x20\x20\x20\x20return RustString(\"{{\\\"ok\\\":\\(result)}}\")\n\
                 \x20   }} catch {{\n\
                 \x20\x20\x20\x20return encodeErrEnvelope(\"\\\\(error)\")\n\
                 \x20   }}\n"
            ));
            ...
            // Non-throwing method
            content.push_str(&format!(
                "        return RustString(inner.{method_camel}({call_args_str}))\n"
            ));
        }
