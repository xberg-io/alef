...
fn emit_lib_rs(
    ...
    // Emit `pub fn <type>_noop` definitions for every type that declared a noop method in
    // its extern block: deferred handle types (declared in the functions block) AND own-block
    // opaque types with no visible methods (declared via `emit_extern_block_for_type`). Without
    // the matching Rust definition in `super`, the bridge declaration fails with E0425.
