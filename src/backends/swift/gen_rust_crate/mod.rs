...
fn emit_lib_rs(
    ...
    // Rust 2024 lints `#[no_mangle]` as `unsafe_attr_outside_unsafe`, but
    // swift-bridge 0.1.59 still depends on syn 1.x, which cannot parse the
    // `#[unsafe(no_mangle)]` form. Until swift-bridge upgrades, emit plain
    // `#[no_mangle]` on extern "C" callback registration fns and silence the
    // 2024-edition deprecation lint at the crate root.
    out.push_str("#![allow(unsafe_attr_outside_unsafe)]\n");
    out.push_str("#![allow(\n");
