...
// ComponentError reports a downloadable-component operation's stable failure code alongside
// its human-readable message, so callers can branch on Code -- "offline", "signature_invalid",
// "unsupported_host", and so on -- instead of parsing free-form text.
...
// ComponentStatus returns ready, cached, not_downloaded, bundled, or unsupported:<reason>.
// See ComponentStatusCode for the matching numeric code.
...
// ComponentStatusCode returns the numeric counterpart to ComponentStatus, stable across
// releases: 0 ready, 1 cached, 2 not_downloaded, 3 bundled, 4 unsupported.
func ComponentStatusCode(component string) (int32, error) {{
	cComponent := C.CString(component)
	defer C.free(unsafe.Pointer(cComponent))
	result := C.{ffi_prefix}_component_status_code(cComponent)
	if result < 0 {{
		return 0, componentError("component status code")
	}}
...
func componentError(operation string) error {{
	...
	// The native layer prefixes every component failure message with a stable
	// "code: " token (see alef_component_error_message in alef::backends::native_components);
	// splitting it back out here is what lets a caller type-assert *ComponentError and
	// branch on Code instead of matching on the message text.
	code, message, found := strings.Cut(err.Error(), ": ")
	if !found {{
		return fmt.Errorf("%s: %w", operation, err)
	}}
	return fmt.Errorf("%s: %w", operation, &ComponentError{{Code: code, Message: message}})
}}
"#,
    );
...
mod tests {
    ...
    fn emits_go_wrappers_over_shared_c_manager() {
        ...
        assert!(generated.contains("func ComponentStatusCode(component string) (int32, error)"));
        assert!(generated.contains("C.demo_component_status_code(cComponent)"));
        ...
        assert!(generated.contains("type ComponentError struct"));
        assert!(generated.contains("strings.Cut(err.Error(), \": \")"));
    }
}
