...
pub fn inject_display_as_text_methods(source: &str, text_type_names: &[String]) -> String {
    ...
    for type_name in text_type_names {
        // Find the sealed class declaration and inject the text() getter after the class.
        // The sealed class looks like: `sealed class <TypeName> {`
        // For FRB sealed classes with variants, the structure after FRB generation is:
        // ```dart
        // sealed class AssistantContent {
        //   const factory AssistantContent.text({required String field0}) = AssistantContent_Text;
        //   const factory AssistantContent.parts({required List<ContentPart> field0}) = AssistantContent_Parts;
        // }
        // ```
        //
        // After variant rewriting by alef, it becomes:
        // ```dart
        // sealed class AssistantContent {
        //   const factory AssistantContent.text({required String text}) = AssistantContent_Text;
        //   const factory AssistantContent.parts({required List<ContentPart> parts}) = AssistantContent_Parts;
        // }
        // ```
        //
        // We need to inject a text() method into the sealed class. However, sealed classes
        // in Dart are generated by FRB and don't allow adding methods directly. Instead,
        // we use an extension to add the method (Dart extension pattern).

        // Match sealed class with optional mixin (e.g., `with _$AssistantContent` for freezed integration)
        let sealed_class_pattern_exact = format!("sealed class {} {{", type_name);
        let sealed_class_pattern_with_mixin = format!("sealed class {} with", type_name);

        let found = result.contains(&sealed_class_pattern_exact) || result.contains(&sealed_class_pattern_with_mixin);
        if found {
...
fn generate_text_method_extension(type_name: &str) -> String {
    ...
    //
    // The actual variants depend on the type definition, but we can generate a
    // generic extension that works for common patterns:
    //
    // extension <TypeName>TextExt on <TypeName> {
    //   /// Returns the plain-text display value of this content.
    //   /// - If the variant contains a String field, it's returned verbatim.
    //   /// - If the variant contains a List (content parts), text parts are concatenated.
    //   /// - Otherwise returns an empty string.
    //   String text() {
    //     return switch (this) {
    //       <TypeName>_Text(:final text) => text,
    //       <TypeName>_Parts(:final parts) => _extractTextFromParts(parts),
    //       _ => '',
    //     };
    //   }
    // }
  ...
  /// - If this is a Parts variant containing content elements, concatenates the text
  ///   fields of all elements whose `type` equals "text", skipping non-text parts.
  /// - Otherwise returns an empty string.
  String text() {{
    return switch (this) {{
...
/// Each part is expected to have a `type` field and a `text` field.
