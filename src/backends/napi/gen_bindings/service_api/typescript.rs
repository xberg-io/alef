...
pub(in crate::backends::napi::gen_bindings) fn gen_service_ts(
    ...
    if !service_names.is_empty() {
        // Re-export the service class(es) plus the native value types the service ~keep
        // API references (enums, builder types) so consumers importing from the ~keep
        // service module can construct arguments (e.g. `Method.Get`, `new ~keep
        // RouteBuilder(...)`) without a second import from the native index. The ~keep
        // service's own aliased self-import (`X as NativeX`) is internal and skipped. ~keep
