...
/// Generate TypeScript trait bridge interfaces and adapter classes for Path A pattern.
///
/// For each trait bridge, generates:
/// 1. A TypeScript interface `<Trait>Bridge` that users implement
/// 2. An adapter class `<Trait>Adapter` that wraps user implementations and handles marshalling
///
/// Returns Vec<(filename, content)> ready for emission into the TypeScript package.
