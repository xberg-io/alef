...
"#,
    ...
    // Stub Swift source for the user-facing module target.
    // SwiftPM requires at least one source file in every target listed in Package.swift.
    // Without this file, the `{module}` target has no sources and SwiftPM cannot register
    // the product `.library(name: "{module}", targets: ["{module}"])`, causing:
    //   error: product 'Module' required by target 'ModuleTests' not found in package 'Module'
    // The `@_exported import RustBridge` re-exports the bridge so callers only need
    // `import {module}`.
    let module_stub_swift = format!(
        "// {module}: public Swift API surface for the Rust binding.\n\
         // This file is generated by alef-scaffold — extend with high-level wrappers around RustBridge as needed.\n\
         @_exported import RustBridge\n",
        module = module,
    );
...
"#,
    ...
    Ok(vec![
        ...
        GeneratedFile {
            path: PathBuf::from(format!("packages/swift/Sources/{module}/{module}.swift")),
            content: module_stub_swift,
            generated_header: false,
        },
        GeneratedFile {
