    ...
    fn generate(
        ...
        for group in groups {
            let active: Vec<&Fixture> = group
                .fixtures
                .iter()
                // Skip http-type fixtures — Gleam tests only cover call-based fixtures
                // (those with function arguments). HTTP server tests require an HTTP
                // client library integration that isn't part of the Gleam package.
                .filter(|f| !f.is_http_test())
                // Skip fixtures that don't have a Gleam-specific call override pointing
                // to an exported function. The default `handle_request` function does not
                // exist in the Gleam binding (which only exports schema construction
                // functions). Only fixtures with an explicit `call` config that resolves
                // to a gleam override should generate tests.
                .filter(|f| {
        ...
        // When no fixture-driven tests were generated (e.g., the binding only exposes
        // schema construction functions, not handler functions), emit a minimal smoke
        // test so `gleam test` still succeeds and compilation is verified.
        if !any_tests {
            let smoke = concat!(
                "// Generated by alef. Do not edit by hand.\n",
                "// No fixture-driven tests for Gleam — binding only exports schema construction functions\n",
                "// that require the Elixir NIF to be loaded at runtime. This smoke test verifies\n",
                "// that the package compiles and is importable without executing NIF calls.\n",
                "import gleeunit\n",
                ...
                "  // The Gleam binding compiles correctly and is a valid Erlang module.\n",
                "  // NIF-backed functions are not called here because they require the\n",
