...
impl E2eCodegen for GleamE2eCodegen {
    fn generate(
        ...
        // Emit a helper module with a `read_file_bytes` external so tests can
        // pass `BitArray` content to `extract_bytes_sync` et al.
...
fn render_test_file(
    ...
    for fixture in fixtures {
        ...
        // Optional string args emit option.Some(...) — need option import.
        let has_optional_string_arg = call_config
            .args
            .iter()
            .any(|a| a.arg_type == "string" && a.optional);
        if has_bytes_arg || has_optional_string_arg {
...
///   option.Some(kreuzberg.Excel(e)) -> e.sheet_count |> option.unwrap(0) |> int.is_at_least(2) |> should.equal(True)
...
fn render_tagged_union_assertion(
    ...
    let _ = writeln!(out, "    option.Some({module_qualifier}.{constructor}({inner_var})) -> {{");
    ...
    match assertion.assertion_type.as_str() {
        ...
        "contains" => {
            if let Some(expected) = &assertion.value {
                ...
                if suffix_is_array {
                    ...
                    let _ = writeln!(
                        out,
                        "      let items__ = {inner_field_expr} |> option.unwrap([])"
                    );
                    let _ = writeln!(
                        out,
        ...
        "greater_than_or_equal" => {
            if let Some(val) = &assertion.value {
                ...
                if suffix_is_optional {
                    let _ = writeln!(
                        out,
                        "      {inner_field_expr} |> option.unwrap(0) |> int.is_at_least({gleam_val}) |> should.equal(True)"
                    );
                } else {
                    let _ = writeln!(
                        out,
                        "      {inner_field_expr} |> int.is_at_least({gleam_val}) |> should.equal(True)"
                    );
                }
            }
        }
        "greater_than" => {
            if let Some(val) = &assertion.value {
                ...
                if suffix_is_optional {
                    let _ = writeln!(
                        out,
                        "      {inner_field_expr} |> option.unwrap(0) |> int.is_strictly_greater_than({gleam_val}) |> should.equal(True)"
                    );
                } else {
                    let _ = writeln!(
                        out,
                        "      {inner_field_expr} |> int.is_strictly_greater_than({gleam_val}) |> should.equal(True)"
                    );
                }
            }
        }
        "less_than" => {
            if let Some(val) = &assertion.value {
                ...
                if suffix_is_optional {
                    let _ = writeln!(
                        out,
                        "      {inner_field_expr} |> option.unwrap(0) |> int.is_strictly_less_than({gleam_val}) |> should.equal(True)"
                    );
                } else {
                    let _ = writeln!(
                        out,
                        "      {inner_field_expr} |> int.is_strictly_less_than({gleam_val}) |> should.equal(True)"
                    );
                }
            }
        }
        "less_than_or_equal" => {
            if let Some(val) = &assertion.value {
                ...
                if suffix_is_optional {
                    let _ = writeln!(
                        out,
                        "      {inner_field_expr} |> option.unwrap(0) |> int.is_at_most({gleam_val}) |> should.equal(True)"
                    );
                } else {
                    let _ = writeln!(
                        out,
                        "      {inner_field_expr} |> int.is_at_most({gleam_val}) |> should.equal(True)"
                    );
                }
            }
        }
            ...
            if let Some(val) = &assertion.value {
                if let Some(n) = val.as_u64() {
                    if suffix_is_optional {
                        let _ = writeln!(
                            out,
                            "      {inner_field_expr} |> option.unwrap([]) |> list.length |> int.is_at_least({n}) |> should.equal(True)"
                        );
                    } else {
                        let _ = writeln!(
                            out,
                            "      {inner_field_expr} |> list.length |> int.is_at_least({n}) |> should.equal(True)"
                        );
                    }
                }
            }
        ...
        "not_empty" => {
            if suffix_is_optional {
                ...
                let _ = writeln!(
                    out,
                    "      {inner_field_expr} |> list.is_empty |> should.equal(False)"
                );
            }
        }
        "is_empty" => {
            if suffix_is_optional {
                ...
                let _ = writeln!(
                    out,
                    "      {inner_field_expr} |> list.is_empty |> should.equal(True)"
                );
            }
        }
        "is_true" => {
        ...
        other => {
            let _ = writeln!(out, "      // tagged-union assertion '{other}' not yet implemented for Gleam");
        }
    }
