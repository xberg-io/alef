...
use crate::gen_rust_crate::type_bridge::{
    bridge_type, bridge_type_enum_aware, bridge_type_with_handles, needs_json_bridge,
};
...
pub(crate) fn emit_extern_block_for_type(
    ...
    for field in &ty.fields {
        ...
        let bridge_ty = if field.optional && !needs_json_bridge(&field.ty) {
            // Option<Vec<String>> is not supported; fall back to plain String (JSON).
...
pub(crate) fn emit_extern_block_for_enum(_en: &EnumDef) -> String {
    // Enums are NOT declared as opaque types in the swift-bridge extern block.
    //
    // swift-bridge 0.1.59 generates `extension T: Vectorizable` (with
    // `__swift_bridge__$Vec_T$*` C-ABI symbols) for every opaque
    // `extern "Rust" { type T; }` declaration — even when T is a Rust enum.
    // The Rust proc macro does NOT generate the corresponding C symbols for
    // Rust enums, so any crate that declares an opaque enum type gets a linker
    // error when the generated Swift calls `__swift_bridge__$Vec_T$pop` etc.
    //
    // Instead, getters that return enum-typed fields return `String` (the serde
    // variant name) by calling `to_string()` on the wrapper enum. The
    // `to_string()` method on the wrapper is still generated by `enums.rs` and
    // used internally by struct getters; it is no longer exposed across the FFI
    // as an extern "Rust" function because the enum type is not declared there.
    String::new()
}
