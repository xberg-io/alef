    ...
    fn generate_bindings(&self, api: &ApiSurface, config: &ResolvedCrateConfig) -> anyhow::Result<Vec<GeneratedFile>> {
        ...
        let param_is_owned_struct = |ty: &alef_core::ir::TypeRef| -> bool {
            let is_non_opaque_struct = |n: &str| {
                !opaque_types.contains(n) && !enum_names.contains(n) && !arc_incompatible_opaque.contains(n)
            };
            match ty {
        ...
        let method_references_enum = |m: &alef_core::ir::MethodDef| -> bool {
            references_enum(&m.return_type)
                || m.params.iter().any(|p| references_enum(&p.ty) || param_is_owned_struct(&p.ty))
        };
        ...
        let is_extendr_native_incompatible = |ty: &alef_core::ir::TypeRef| -> bool {
            ...
            let is_vec_element_incompatible = |n: &str| {
                !opaque_types.contains(n) && !enum_names.contains(n) && !arc_incompatible_opaque.contains(n)
            };
            match ty {
        ...
        for typ in api.types.iter().filter(|typ| !typ.is_trait) {
            if typ.is_opaque {
                ...
                if extendr_incompatible_types.contains(&struct_typ.name) {
                    ...
                    let impl_block = generators::gen_impl_block(&struct_typ, self, &cfg, &adapter_bodies, &opaque_types);
                    if !impl_block.is_empty() {
                        builder.add_item(&impl_block);
                    } else {
