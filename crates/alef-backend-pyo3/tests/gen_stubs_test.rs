...
fn test_basic_stubs() {
    ...
    // No Literal/TypedDict/TypeAlias/Any in a plain struct + simple enum, so no typing import.
    assert!(
        !content.contains("from typing import"),
        "Plain struct+enum with no Any/Literal/TypedDict/TypeAlias should not emit typing import"
    );
...
fn make_config_with_stubs_and_capsule_types() -> ResolvedCrateConfig {
    let cfg: NewAlefConfig = toml::from_str(
        r#"
[workspace]
languages = ["python"]
...
/// Capsule types must not appear as class stubs in the `.pyi` file — they live in third-party
/// packages (e.g. `tree_sitter`). Functions returning a capsule type must use `Any` instead.
#[test]
fn test_capsule_types_not_declared_as_classes_in_stubs() {
    let backend = Pyo3Backend;
    ...
    let api = ApiSurface {
        ...
        version: "0.1.0".to_string(),
        types: vec![
            // Language is a capsule type — must NOT emit a class stub.
            TypeDef {
    ...
    let config = make_config_with_stubs_and_capsule_types();
    let result = backend.generate_type_stubs(&api, &config).unwrap();
    let content = result.into_iter().next().unwrap().content;

    // The capsule type must NOT appear as a class declaration.
    assert!(
        !content.contains("class Language:") && !content.contains("class Language ..."),
        "Capsule type Language must not be declared as a class in the stub; content:\n{content}"
    );

    // The function returning a capsule type must use `Any` as the return type.
    assert!(
        content.contains("def get_language(name: str) -> Any:"),
        "Function returning capsule type must use Any as return type; content:\n{content}"
    );

    // The typing import must be conditional — since `Any` is used, it must appear.
    assert!(
        content.contains("from typing import Any"),
        "from typing import Any must be emitted when Any is used; content:\n{content}"
    );

    // The import must NOT include unused names (Literal, TypeAlias, TypedDict).
    assert!(
        !content.contains("Literal") && !content.contains("TypedDict") && !content.contains("TypeAlias"),
        "Typing import must contain only names that are actually used; content:\n{content}"
    );
}

/// A function that takes an Optional<CapsuleType> must emit `Any | None` in its stub signature.
#[test]
fn test_optional_capsule_param_uses_any_or_none_in_stubs() {
    let backend = Pyo3Backend;
    ...
    let api = ApiSurface {
        ...
        functions: vec![FunctionDef {
            ...
            rust_path: "test_lib::set_language".to_string(),
            original_rust_path: String::new(),
            params: vec![ParamDef {
                name: "lang".to_string(),
                ty: TypeRef::Optional(Box::new(TypeRef::Named("Language".to_string()))),
                optional: true,
                default: None,
                sanitized: false,
                typed_default: None,
    ...
    // Optional capsule param must be `Any | None`, not `Language | None`.
    assert!(
        content.contains("lang: Any | None = None"),
        "Optional capsule param must use Any | None; content:\n{content}"
    );
    assert!(
        !content.contains("Language"),
        "Capsule type name Language must not appear in stubs; content:\n{content}"
    );
}
