...
- **Version-idempotent verify** -- `alef verify` is a pure read+strip+rehash+compare. The hash baked into every generated file is `blake3(sources_hash || file_content_without_hash_line)` -- per-file, derived from the rust sources and the on-disk byte content; deliberately *no* alef CLI version dimension. Upgrading the alef CLI does not by itself invalidate verify on a tagged repo. `alef generate` finalises the embedded hash after every formatter has run, so the on-disk hash always matches the on-disk content. See [Verify model](#verify-model) below for details.
